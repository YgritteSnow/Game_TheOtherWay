...
public class EventDelegate<T>
{
	...
	public override bool Equals(object obj)
	{
		...
		if (obj is EventDelegate<T>)
		{
			EventDelegate<T> del = obj as EventDelegate<T>;
			return (mTarget == del.mTarget && string.Equals(mMethodName, del.mMethodName));
		}
		return false;
	}

	static int s_Hash = "EventDelegate<T>".GetHashCode();
	...
	void Cache()
	{
		...
		if (mCachedCallback == null || (mCachedCallback.Target as MonoBehaviour) != mTarget || GetMethodName(mCachedCallback) != mMethodName)
		{
			if (mTarget != null && !string.IsNullOrEmpty(mMethodName))
			{
				...
				if (mMethod.ReturnType != typeof(T))
				{
					Debug.LogError(mTarget.GetType() + "." + mMethodName + " must have a '" + typeof(T) + "' return type.", mTarget);
					return;
				}
	...
	public T Execute()
	{
		...
		if (mCachedCallback != null)
		{
			...
			else if (mCachedCallback.Target != null)
			{
				...
				// #endif
				if (objs != null && objs.Length > 0) result = mCachedCallback();
			}
#endif
			return result;
		}
	...
	static public void Execute(List<EventDelegate<T>> list)
	{
		if (list != null)
		{
			for (int i = 0; i < list.Count;)
			{
				EventDelegate<T> del = list[i];
	...
	static public bool IsValid(List<EventDelegate<T>> list)
	{
		if (list != null)
		{
	...
	static public EventDelegate<T> Set(List<EventDelegate<T>> list, Callback callback)
	{
		if (list != null)
		{
			EventDelegate<T> del = new EventDelegate<T>(callback);
			list.Clear();
			list.Add(del);
			return del;
		}
	...
	static public void Set(List<EventDelegate<T>> list, EventDelegate<T> del)
	{
		if (list != null)
		{
			list.Clear();
	...
	static public EventDelegate<T> Add(List<EventDelegate<T>> list, Callback callback) { return Add(list, callback, false); }

	/// <summary>
	/// Append a new event delegate to the list.
	/// </summary>

	static public EventDelegate<T> Add(List<EventDelegate<T>> list, Callback callback, bool oneShot)
	{
		if (list != null)
		{
			for (int i = 0, imax = list.Count; i < imax; ++i)
			...
			EventDelegate<T> ed = new EventDelegate<T>(callback);
			ed.oneShot = oneShot;
			list.Add(ed);
			return ed;
		}
	...
	static public void Add(List<EventDelegate<T>> list, EventDelegate<T> ev) { Add(list, ev, ev.oneShot); }

	/// <summary>
	/// Append a new event delegate to the list.
	/// </summary>

	static public void Add(List<EventDelegate<T>> list, EventDelegate<T> ev, bool oneShot)
	{
		...
		else if (list != null)
		{
			...
			EventDelegate<T> copy = new EventDelegate<T>(ev.target, ev.methodName);
	...
	static public bool Remove(List<EventDelegate<T>> list, Callback callback)
	{
		if (list != null)
		{
			for (int i = 0, imax = list.Count; i < imax; ++i)
	...
	static public bool Remove(List<EventDelegate<T>> list, EventDelegate<T> ev)
	{
		if (list != null)
		{
			for (int i = 0, imax = list.Count; i < imax; ++i)
...
[System.Serializable]
public class EventDelegateFloat : EventDelegate<float> { }